...
$noapk  = $_SESSION['noapk'];
  if(isset($_POST['del'])){
    ...
    //         </div>";
    // }else {
    // $insQry = "DELETE FROM rsekolah WHERE idkota = ? AND noapk = $_SESSION[noapk]";
	...
	// 	mysqli_stmt_execute($stmt) or die ("Gagal Query Hapus Kota : " . mysqli_error($koneksidb));
	// 	mysqli_stmt_close($stmt);

    // logTransaksi($iduser, date('Y-m-d H:i:s'), 'Data Kota Dihapus', $noapk);
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" class="form-horizontal">
  <div class="portlet box <?= $_SESSION['warnabar'] ?>">
    <div class="portlet-title">
        <div class="caption">Ubah Data Profile Sekolah</div>
        <div class="actions">
            <!-- <a href="?content=ProfilSekolah" class="btn  <?= $_SESSION['warnatombol'] ?>"><i class="fa fa-plus-circle"></i> Tambah Data</a>   -->
        </div>
    </div>
      ...
      <table class="table table-condensed table-bordered table-hover table-condensed" id="sample_2" width="100%">
        <thead>
          <tr class="active">
                ...
                <td>Nama Provinsi</td>
                <td>Kode Pos</td>
                <td width="1%"><div align="center">ACTION</div></td>
              </tr>
        </thead>
