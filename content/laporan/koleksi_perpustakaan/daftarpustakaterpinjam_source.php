...
$tampil             = (isset($_SESSION['tampil'])) ? $_SESSION['tampil'] : "";
    ...
    //nama table database
    $sTable = " (SELECT a.idbuku AS idbuku, b.judul AS judul, a.nipnis AS nipnis, c.nama AS nama, a.tglpinjam AS tglpinjam, a.tglhrskembali AS tglhrskembali, a.isterlambat AS isterlambat FROM tpinbuku a JOIN tbuku b ON a.idbuku = b.idbuku JOIN ranggota c ON a.nipnis = c.nipnis AND a.iskembali = 0 AND b.idjnsbuku < 4 AND a.noapk = $_SESSION[noapk]) AS tpinbuku ";
    //primary key
    $sIndexColumn = "idbuku";
...
$no = $_GET['iDisplayStart'] + 1;
	...
	{    
        ...
        if($tglhrskembali < date("Y-m-d")){
            $isterlambat = "YA";
        }else{
            $isterlambat = "TIDAK";
        }
       
        $row = array($no, $idbuku, $judul,$nipnis,$nama,$tglpinjam,$tglhrskembali,$isterlambat);  
