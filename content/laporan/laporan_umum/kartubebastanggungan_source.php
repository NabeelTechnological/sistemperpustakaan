...
$sTable = "(SELECT a.noapk, a.idbuku AS idbuku, a.judul AS judul, b.tglpinjam AS tglpinjam, b.tglhrskembali AS tglhrskembali,b.iskembali AS iskembali , b.nipnis AS nipnis, c.idjnsang AS idjnsang
FROM tbuku a JOIN tpinbuku b ON a.idbuku = b.idbuku JOIN ranggota c ON b.nipnis = c.nipnis) AS tpinbuku";
$sIndexColumn = "idbuku";

if($dataAnggota=="Siswa"){
    $sWhereDefault = "WHERE idjnsang = 1 ";

}else if($dataAnggota=="Guru/Karyawan"){
    $sWhereDefault = "WHERE idjnsang = 2 ";
}

$sWhereDefault .= "AND iskembali = 0 AND nipnis = $dataIdAnggota AND noapk = $_SESSION[noapk]";
...
while ($dataRow = mysqli_fetch_array($rResult)) {
        ...
        $terlambat = (date("Y-m-d") > $tglhrskembali) ? "YA" : "TIDAK";

        $row = array($no, $idbuku, $judul ,$tglpinjam, $tglhrskembali,$terlambat);
